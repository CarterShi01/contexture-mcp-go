...
var _ = demo.GetPodEvents
var _ = demo.GetPodLogs
var _ = demo.GetPodStatus
var _ = demo.GetRolloutStatus
var _ = demo.RollBackDeployment
...
func main() {
    ...
    if contexture.PackageName != "contexture" || contexture.Version != "0.16.0rc1" || contexture.ReferenceSeparator != "/" {
    ...
    _, _ = manager.RegisterRole(func() *contexture.Role {
        return &contexture.Role{Name: "operations", Description: "Operate.", Instructions: "Inspect.", PreProcess: func() *contexture.PreProcess { return &contexture.PreProcess{Name: "prepare", Description: "Prepare.", Instructions: "Check inputs."} }, PostProcess: func() *contexture.PostProcess { return &contexture.PostProcess{Name: "publish", Description: "Preserve.", Instructions: "Save evidence."} }}
    })
    application, _ := manager.Application("consumer")
    index, _ := contexture.Compile(application)
    processDisclosure, _ := contexture.NewDisclosure(index, contexture.AllRoots()); processOwner, _ := processDisclosure.Open("operations", contexture.AllRoots()); if processOwner["pre_process"] != "operations/prepare" || processOwner["post_process"] != "operations/publish" { panic("public process facade is incomplete") }
    marked, markedErr := contexture.BindingInstruction("consumer policy", "Follow the rule.", "Check it."); if markedErr != nil || !strings.Contains(marked, ">>> REQUIRED: Check it.") { panic("public BindingInstruction is incomplete") }
